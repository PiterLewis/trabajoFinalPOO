...
public class Batalla {
	private List<Personaje> jugadoresHumanos;
    private List<Personaje> jugadoresMaquina;
    private int nivelDificultad; // 1: Fácil, 2: Medio, 3: Difícil

    public Batalla(int nivelDificultad) {
        this.jugadoresHumanos = new ArrayList<>();
        this.jugadoresMaquina = new ArrayList<>();
        this.nivelDificultad = nivelDificultad;
    }

    public void agregarJugadorHumano(Personaje jugador) {
        jugadoresHumanos.add(jugador);
    }

    public void agregarJugadorMaquina(Personaje jugador) {
    ...
    public void iniciar() {
        ...
        while (!jugadoresHumanos.isEmpty() && !jugadoresMaquina.isEmpty()) {
        	...
        	Personaje humano = jugadoresHumanos.get(random.nextInt(jugadoresHumanos.size()));
        	Personaje maquina = jugadoresMaquina.get(random.nextInt(jugadoresMaquina.size()));
    ...
    public void guardarResultado(String archivo) {
        try (FileWriter writer = new FileWriter(archivo)) {
            ...
            for (Personaje jugador : jugadoresHumanos) {
                writer.write(jugador + "\n");
            }
            writer.write("Jugadores Máquina:\n");
            for (Personaje jugador : jugadoresMaquina) {
                writer.write(jugador + "\n");
            }
            writer.write("\n--- FIN DEL RESULTADO ---\n");
